...
const DocumentCard = ({document, onDelete}) => {
  ...
  return (
    <div onClick={handleNavigate} className="group relative bg-gray-900 backdrop-blur-3xl border rounded-2xl p-5 shadow-primary shadow-md hover:shadow-lg transition-all duration-300 flex flex-col justify-between cursor-pointer hover:-translate-y-1 hover:bg-secondary/30">
      {/* Header */}
      <div>
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          <button onClick={handleDelete} className="opacity-0 group-hover:opacity-100 justify-center hover:bg-red-500 transition-colors duration-200 p-2 rounded-lg">
            <Trash2 className="h-5 w-5" strokeWidth={2} />
          </button>
        </div>

        {/* Title */}
        <h3 title={document.title} className="text-base font-semibold truncate mb-2">{document.title}</h3>
